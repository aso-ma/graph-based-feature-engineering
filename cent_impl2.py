...
class Constants:
    ...
    # Type of each feature in the data (continuous, nominal, ordinal)
    # Use nominal for binary and unordered categorical features
    COL_TYPES = {
        'Feature_1': 'continuous',
        'Feature_2': 'continuous',
        'Feature_3': 'continuous',
        'Feature_4': 'continuous',
        'Feature_5': 'continuous'
    }

    COMPLETE_GRAPH_FILE_NAME = 'complete_similarity_graph.pkl'
    PRIMARY_GRAPH_FILE_NAME = 'primary_similarity_graph.pkl'

def generate_complete_similarity_graph(preprocessed_df: pd.DataFrame):
    df_feature = preprocessed_df.drop(columns=[Constants.TARGET_COL])
    similarity_matrix = gower_similarity_matrix(df_feature, Constants.COL_TYPES)
    similarity_matrix.to_pickle(Constants.OUT_DIR_PATH + 'complete_sim_mat.pkl')

    similarity_graph = nx.Graph()
    similarity_graph.add_nodes_from(preprocessed_df.index)

    for i, node_i in enumerate(df_feature.index):
        for j, node_j in enumerate(df_feature.index):
            if i < j:
                similarity_graph.add_edge(node_i, node_j, weight=similarity_matrix.at[node_i, node_j])
   
    with open(Constants.OUT_DIR_PATH + Constants.COMPLETE_GRAPH_FILE_NAME, 'wb') as f:
        pickle.dump(similarity_graph, f)

    primary_graph = edge_rm(similarity_graph, Constants.EDGE_RM_TH)

    with open(Constants.OUT_DIR_PATH + Constants.PRIMARY_GRAPH_FILE_NAME, 'wb') as f:
        pickle.dump(primary_graph, f)

    
def generate_similarity_graph_for(dataframe: pd.DataFrame) -> nx.Graph:
    with open(Constants.OUT_DIR_PATH + Constants.PRIMARY_GRAPH_FILE_NAME, 'rb') as f:
        primary_graph: nx.Graph = pickle.load(f)
    primary_node_set = set(primary_graph.nodes())
    diff = primary_node_set.difference(dataframe.index.values)
    if len(diff) > 0:
        primary_graph.remove_nodes_from(diff)
...
class StatsFeatSel:     
    ...
    def comp_significant_feat(dataframe: pd.DataFrame):
        ...
        graph = generate_similarity_graph_for(dataframe)
...
def add_test_node(df_train: pd.DataFrame, graph: nx.Graph, test_node: str | int, test_node_primary_feat: pd.Series):
    ...
    for index, row in df_feature.iterrows():
        similarity = gower_score_for(test_node_primary_feat, row, Constants.COL_TYPES, RangeDict, RankDict)
        edges.append((index, test_node, similarity))
...
if __name__ == "__main__":
    ...
    # initialize gower distance
    initialize(data.drop(columns=Constants.TARGET_COL), Constants.COL_TYPES)
    # generate complete similarity graph
    generate_complete_similarity_graph(data)
    ...
    for fold_num, df_train, df_test in tqdm(fold_data, desc="Processing Folds"):
        ...
        g_train = generate_similarity_graph_for(df_train)
